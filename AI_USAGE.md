...
| `src/V4SettlementHook.sol` | Claude Code | Drafted from `specs/HOOK-SPEC.md` sections 5 and 7d and the pinned OpenZeppelin `BaseHook`; the counter and event exist so the callback's execution is observable |
| `src/UnicaSettlementRouter.sol` | Claude Code | Drafted on day 2 from `specs/HOOK-SPEC.md` sections 2 and 3 and addendum A13, against the pinned `IPoolManager` and `IUnlockCallback`; the native-settle order (sync, then settle) follows `PoolManager.sol`'s own comment |
| `test/V4SettlementHook.t.sol` | Claude Code | Drafted from `specs/THREAT-MODEL.md` T5 and v4-core's own `Deployers` test utilities. The first salt-mining loop ran out of memory and was corrected in the same hour; the guard was sabotaged (beforeSwap flipped on) and seen to fail before it was trusted |
| `test/utils/SettlementTestBase.sol` | Claude Code | Written from the v4 interfaces and hookmate's artifact API on day 2 so the tests deploy Uniswap's official PoolManager bytecode instead of compiling it; replaces the day-1 dependency on v4-core's `Deployers` |
| `test/UnicaSettlementRouter.t.sol`, `test/I7NativeSettle.t.sol`, `test/utils/RouterHarness.sol` | Claude Code | Day 2. The router tests assert shape, never arithmetic; the fuzzer's partial-fill counterexample became its own test. The I7 file is the four-row design from the build card: defence on/off crossed with the foreign-sync precondition present/absent, with the harness making the defence switchable |

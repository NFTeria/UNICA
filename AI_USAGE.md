...
| `test/utils/UnicaTestBase.sol` | Claude Code | Written from the v4 interfaces and hookmate's artifact API on day 2 so the tests deploy Uniswap's official PoolManager bytecode instead of compiling it; replaces the day-1 dependency on v4-core's `Deployers` |
...
| `script/DeployUnica.s.sol`, `script/Interactions.s.sol` | Claude Code | Day 2: thin named entry points over the shared script base, so each stage is one `make` target |

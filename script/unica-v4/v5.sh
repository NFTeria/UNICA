...
# The keystore NAME is the file name under ~/.foundry/keystores. `cast wallet list` on this toolchain prints each name with a
# display "0x" in front of it that the file does not carry, so a name copied from that list is accepted with or without it.
KEYSTORES="$HOME/.foundry/keystores"
need_account() { [ -n "${DEPLOYER_ACCOUNT:-}" ] || { echo "set DEPLOYER_ACCOUNT to one of the keystore names in $KEYSTORES:"; ls "$KEYSTORES"; exit 1; }; if [ ! -f "$KEYSTORES/$DEPLOYER_ACCOUNT" ] && [ -f "$KEYSTORES/${DEPLOYER_ACCOUNT#0x}" ]; then DEPLOYER_ACCOUNT="${DEPLOYER_ACCOUNT#0x}"; fi; [ -f "$KEYSTORES/$DEPLOYER_ACCOUNT" ] || { echo "STOP: no keystore named $DEPLOYER_ACCOUNT in $KEYSTORES; the names there are:"; ls "$KEYSTORES"; exit 1; }; echo "keystore $DEPLOYER_ACCOUNT"; }
...
case "$STEP" in
  ...
  mint) [ "$C" = 46630 ] || fail "the test dollar (uTUSD) exists only on chain 46630; the other test networks use Circle test USDC from a faucet"; need_account; chain; T=$(cfg UNICA_PAYOUT); D=$(cfg DEPLOYER); [ -n "$T" ] && [ -n "$D" ] || fail "UNICA_PAYOUT and DEPLOYER must both be recorded in $CFG"; [ "$(cast call "$T" "MINTER()(address)" --rpc-url "$NET" | tr "[:upper:]" "[:lower:]")" = "$(tr "[:upper:]" "[:lower:]" <<<"$D")" ] || fail "the recorded deployer is not the minter of $T"; cast send "$T" "mint(address,uint256)" "$D" 100000000 --rpc-url "$NET" --account "$DEPLOYER_ACCOUNT"; BAL=$(cast call "$T" "balanceOf(address)(uint256)" "$D" --rpc-url "$NET" | awk '{print $1}'); SEED=$(cfg UNICA_SEED_PAYOUT); echo "uTUSD balance (base units, 6 decimals): $BAL; stage C seeds $SEED"; [ "$BAL" -ge "$SEED" ] || fail "the mint did not land (balance $BAL below the seed $SEED): the keystore is probably not the minter $D" ;;
  ...
  manifest) status_is 4; KEEP=""; if [ -f "$MAN" ]; then KEEP=$(node -e 'const m=JSON.parse(require("fs").readFileSync(process.argv[1],"utf8")); if (m.verification && m.verification.status === "verified") console.log(JSON.stringify(m.verification))' "$MAN"); fi; if [ -n "$KEEP" ]; then UNICA_VERIFICATION_JSON="$KEEP" bash script/unica-v4/manifest.sh "$NET" "$CFG" "$MAN"; else bash script/unica-v4/manifest.sh "$NET" "$CFG" "$MAN"; fi ;;
  ...
  commit) test -f "$MAN" || fail "no manifest"; printf '%s\n' "deploy($NET): UNICA v5 on chain $C, recorded from readback and manifest (TESTNET / NO VALUE)" "" "Stages sent by the owner from a keystore; config recorded by the wrapper; manifest from chain reads." > ".rehearsal/deploy-$C.msg"; git add "$CFG" "$MAN" "broadcast/DeployPublic.s.sol/$C" && { if git diff --cached --name-only | grep -q '^vy/src/unica/'; then git reset -q; fail "a private Vyper file was staged; nothing committed"; fi; } && git commit -F ".rehearsal/deploy-$C.msg" && git push origin HEAD && git log --oneline -1 ;;

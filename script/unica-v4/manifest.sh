...
export UNICA_PARENT_NODE="$( [ -n "${UNICA_PARENT_NAME:-}" ] && cast namehash "$UNICA_PARENT_NAME" || true )"
...
const manifest = {
  ...
  identity: env.UNICA_IDENTITY_AUTHORITY ? {authority: env.UNICA_IDENTITY_AUTHORITY, parentName: env.UNICA_PARENT_NAME || null, parentNode: env.UNICA_PARENT_NODE || null, authorityKind: env.UNICA_ENSV2_RESOLVER ? "EnsV2ResolverAuthority over the chain's ENSv2 permissioned resolver" : "pinned", ensV2Resolver: env.UNICA_ENSV2_RESOLVER || null, ensDeploymentId: env.UNICA_ENS_DEPLOYMENT_ID || null, terminalAdmission: env.UNICA_ADMISSION || null, identityToken: env.UNICA_IDENTITY_TOKEN || null, rendererVersion: env.UNICA_RENDERER_VERSION || null, terminalStatusKey: env.UNICA_TERMINAL_STATUS_KEY || null} : {note: "no identity authority configured; terminal admission is not deployed on this chain"},

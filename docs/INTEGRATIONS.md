...
| Field | |
|---|---|
| **1. Published prize requirement** | Arc/Circle, Launch on Arc Testnet & Push to Mainnet (Start Fresh): "Functional MVP and diagram: Projects must demonstrate a working frontend and backend plus an architecture diagram", video demo, GitHub link, and deployment or deployment-readiness on Arc mainnet by September 30. **Track ruling, 2026-09-08:** UNICA targets Arc's **Best DeFi / Onchain Finance Application**, a from-scratch track. An earlier pass of this file recorded a track named "Best DeFi or Agentic Application" as carrying a "only available to Continuity Track participants" line; that reading is preserved here rather than deleted, because the track names must be re-confirmed against the live prize page before the form is filled, and a silently flipped eligibility claim is exactly the error this document exists to prevent. Arc mainnet is **not live** (Private and Public Mainnet listed as Upcoming; Public Testnet, chain id 5042002, Live — chain id re-observed 2026-09-08 via `eth_chainId` → `0x4cef52`). |

...
| T-CH-8 | **Wrong chain, wrong receiver, wrong workflow, or a replayed/expired price report reaches `ChainlinkCREAdapter` (SO §9) and is accepted anyway.** Scoped to that price adapter specifically — its report carries an observation-timestamp field the admission report of T-CH-8A does not, so the two rows name different mechanisms rather than one restated twice. | `onReport` checks, unmodified from SO §9: `msg.sender == FORWARDER` (`NotForwarder`); workflow id and owner (`WorkflowMismatch`, `WorkflowOwnerMismatch`); `chainId == block.chainid` (`ReportChainMismatch`); `receiver == address(this)` (`ReportReceiverMismatch`); strictly increasing observation time (`ReportNotNewer`); for the Streams route, `expiresAt >= block.timestamp` (`ReportExpired`, SO §8). | Receiver contract (`ChainlinkCREAdapter`) | SPECIFIED, simulation-only (SO §9); not deployed anywhere |
| T-CH-8A | **A replayed or stale admission report reaches a purpose-built admission receiver (`RECEIVER.md`) and is accepted anyway.** `AdmissionReport` (`REPORT-SCHEMA.md` §4) has no observation-timestamp field analogous to T-CH-8's `ReportNotNewer` check; this row names the different mechanism that layer actually has, rather than assuming T-CH-8's row covers it. | Identity checks as T-CH-8's first four (`RECEIVER.md` §5 steps 8-10). Replay itself is resisted **solely** by single-use nonce consumption (`!consumed[orderNonce]`, `RECEIVER.md` §5 step 14) — there is no monotonicity or sequencing check of any kind, because each admission is an independent, single-use grant rather than a sequential feed update. Freshness at the moment an order is actually created is a separate property from replay, closed by `RECEIVER.md` §6 item 2's `quoteExpiry`/`policyExpiry` re-check against the stored admission record (`AdmissionExpired`), not by the nonce. | Receiver contract (purpose-built admission receiver, if one is built) | SPECIFIED, simulation-only (`RECEIVER.md` §5-§6); not deployed anywhere |

...
a short output (I3, and I6 by reverting), and emits the receipt (I2). I7's defence is the
official router's own settle, proven in four rows against a switchable stand-in and a fifth
against the router's deployed bytecode. All seven are TESTED against Uniswap's official
...
|---|---|---|---|---|
| **I1** | Recipient guarantee: output reaches only the registered payout address | `V4SettlementHook._beforeSwap` reverts `NotOfficialPath` unless the sender is the Universal Router, then `NotSettlementExecutor` unless the router's `msgSender()` is the executor at its derived address; the executor's plan `take`s the whole output to `order.recipient` from storage it wrote; the executor accepts an order only for a pool its hook guards (`PoolNotGuarded`) and never for the router's sentinel addresses 1 and 2 (`ReservedRecipient`) | a swap from the official test router → wrapped `NotOfficialPath`; the official router driven by a stranger → wrapped `NotSettlementExecutor(stranger)`, the stranger keeps every wei; the positive path asserts the recipient's balance delta equals the receipted output and nobody else, the executor and the router included, received | **TESTED** (`test_RevertWhen_SwapSenderIsNotTheOfficialRouter`, `test_RevertWhen_OfficialRouterIsDrivenByAStranger`, `test_SettlementDeliversToTheRegisteredRecipient`, `test_RevertWhen_OrderNamesAPoolTheHookDoesNotGuard`, `test_RevertWhen_RecipientIsARouterSentinel`, fuzz 10,000) |
| **I2** | Fee transparency: net, fee, recipient id, order id in one receipt event, plus the standard `HookFee` event (addendum A4) | `V4SettlementHook._receipt`: `SettlementReceipt` schema v1 (`docs/RECEIPT-SCHEMA.md`: order id, pool, recipient, version, payer, executor, both currencies, both amounts, fee, reserved policy id) with the payer read from the order the executor recorded, and OpenZeppelin `IHookEvents.HookFee(poolId, payer, 0, 0)`; the fee is zero and says so. A receipt survives only in a transaction where the executor confirmed the recipient received at least the minimum | positive: the test decodes both events and asserts exactly one of each, every field against the order and the recipient's balance; negative controls: every refusal asserts `receiptCount` unchanged, a two-swap plan for one order is refused at the second swap (`OrderAlreadySwapped`), and a token that delivers less than the pool credited reverts the payment and the receipt with it (`RecipientShort`) | **TESTED** (`test_ReceiptCarriesTheOrderAndTheStandardEvent`, `test_RevertWhen_OrderIsSwappedTwiceInOnePlan`, `test_RevertWhen_RecipientReceivesLessThanTheMinimum_FeeOnTransfer`) |
| **I3** | Slippage floor: realised output ≥ the order's minimum; exact-input partial fills revert (addendum A13) | the hook in `_afterSwap`, from the swap's delta against the order it reads: `PartialFill` if the pool consumed less than `amountIn`, `OutputBelowMinimum` if the output is short; `_beforeSwap` refuses a swap whose direction, amount, or pool is not the order's; the executor, after the router returns, refuses an order whose recipient balance grew by less than the minimum (`RecipientShort`), which is the floor where it is felt. The official router also checks its own `amountOutMinimum`, after the hook, which no test can distinguish and is kept as redundancy | output below minimum → wrapped `OutputBelowMinimum`, nothing moves; pool cannot fill → wrapped `PartialFill`, nothing moves; direction, amount or pool differs → `ParamsDoNotMatchOrder`, `PoolDoesNotMatchOrder`; a fee-on-take token → `RecipientShort` with exact numbers | **TESTED** (`test_RevertWhen_OutputBelowMinimum_NothingMoves`, `test_RevertWhen_PoolCannotFillTheOrder_NothingMoves`, `test_RevertWhen_SwapDirectionDisagreesWithTheOrder`, `test_RevertWhen_SwapParamsDisagreeWithTheOrder`, `test_RevertWhen_PoolDisagreesWithTheOrder`, `test_RevertWhen_RecipientReceivesLessThanTheMinimum_FeeOnTransfer`) |
...
| **I5** | Replay protection: one order id settles at most once | the executor moves the order to `Paying` before any external call and to `Settled` after; the hook admits a swap only for an order that is `Paying` right now, so an unknown, open, or settled id is refused whatever the executor says; within one transaction the hook marks an order swapped in transient storage and refuses a second swap for it; ids are `keccak256(chainid, executor, creator, salt)` and a reused salt is refused | pay twice → `OrderNotOpen(Settled)`; a swap naming an unknown, an open, or a settled order → wrapped `OrderNotInFlight` with the state read; two swaps in one plan → wrapped `OrderAlreadySwapped`; a reused salt → `OrderExists` | **TESTED** (`test_RevertWhen_OrderPaidTwice`, `test_RevertWhen_OrderIsNotInFlight`, `test_RevertWhen_OrderIsSwappedTwiceInOnePlan`, `test_OrderIdsAreChainBoundCreatorBoundAndKnownInAdvance`) |

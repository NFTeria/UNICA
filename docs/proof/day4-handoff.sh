...
git diff --quiet day5-green -- src script foundry.toml remappings.txt || { echo "src/, script/, foundry.toml or remappings.txt differ from the day5-green tag; the addresses would move"; exit 1; }
...
echo "chain 11155111, creation code equals day5-green, nonce $nonce, targets vacant, prediction matches: go"
